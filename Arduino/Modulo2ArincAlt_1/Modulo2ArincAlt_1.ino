...
#include <TimerOne.h> // used for Arinc signal timing

const byte    pinLed       =   13; // Arduino's build-in LED, here NOT READY
...
void setup() 
{
  ...
  digitalWrite(pinLed, LOW); // sync indicator
  ...
  PRR = 0b10110111; // only timer1 and timer2 are powered
  ...
  //Timer1.initialize(136000); // 135000 is 135ms update rate
  //Timer1.attachInterrupt(transmitWord); // attach the service routine
...
void loop() {
  ...
  digitalWrite(pinLed, mod2Synch); // indicate sync bit found
